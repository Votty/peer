...
const help = (arg, pkg, log) => {
    let isHelp = arg.fn.keyword === "help" && arg.obj.length === 0;
...
const parse = async function(e) {
    ...
    return new Promise((ok, no) => {
        ...
        for (var f in cs.APP_FUNC) {
            ...
            for (var i in cs.APP_FUNC[f].alias)
                avKeywords[cs.APP_FUNC[f].alias[i]] = cs.APP_FUNC[f];
        }

        // Gather all available options of main function
        let optKeywords = [];
        for (var o in avKeywords[cs.APP_DEFFUNC].options)
            optKeywords = optKeywords.concat(
                cs.APP_OPT[avKeywords[cs.APP_DEFFUNC].options[o]].flags
            );
        ...
        let processArgs = (v, i, a) => {
            if (i == 0) {
                ...
                if (typeof optDataComes == "string") {
                    ...
                    if (_reqIdx >= 0) analyzed._reqOptList.splice(_reqIdx, 1);
        ...
        if (typeof optDataComes === "string")
            return no(
                e.make(
                    0x5,
                    `Option value for '${optDataComes}' option is missing.`
                )
            );
